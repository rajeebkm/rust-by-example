...
// for `print`.
fn apply_x<F>(f: F) where
    F: Fn() {    //F: FnOnce(){ f(); } can be used here or, F: FnMut() { f(); } can be used by modifying parameter as (mut f: F).
    f();
}

fn apply_y<F>(f: F) where
    F: FnOnce() {       // F: Fn(){ f(); } cann't be implemented. As, the closure implements `FnOnce`, not `Fn` here, closure is `FnOnce` because it moves the variable `y` out of its environment
    f();
}
...
fn main() {
    ...
    let printy = || { println!("{}", y);  // || <- this closure implements `FnOnce`, not `Fn`
    mem::drop(y);  // closure is `FnOnce` because it moves the variable `y` out of its environment
};
    apply_y(printy);
}
